...
class LanguageManager {
    constructor() {
        ...
        this.translations = {
            en: {
                ...
                // Notifications
                uploadBothError:
                    "Please upload both followers and following files",
            ...
            id: {
                ...
                heroSubtitle:
                    "Ungkap siapa yang tidak follow balik dengan platform analisis canggih kami. Pribadi, aman, dan dirancang untuk mereka yang menghargai pengalaman premium.",
                startAnalysis: "Mulai Analisis",

                // Features
                feature1Title: "Analisis Akurat",
                feature1Desc:
                    "Deteksi presisi pengikut tidak timbal balik dengan algoritma canggih",
                feature2Title: "Laporan Premium",
                feature2Desc:
                    "Laporan PDF yang dirancang indah dengan wawasan komprehensif",
                feature3Title: "Aman & Pribadi",
                feature3Desc:
                    "Data Anda tidak pernah meninggalkan browser. Privasi lengkap terjamin",

                // Upload Section
                uploadTitle: "Unggah Data Instagram Anda",
                uploadSubtitle:
                    "Pilih file followers.json dan following.json yang diekspor dari Instagram untuk memulai analisis premium",
                ...
                analyzing:
                    "Menganalisis jaringan Instagram Anda dengan algoritma premium...",

                // Results Section
                resultsTitle: "Analisis Premium Selesai",
                resultsSubtitle:
                    "Wawasan komprehensif keterlibatan Instagram Anda:",
                ...
                perfectBalanceDesc:
                    "Jaringan Instagram Anda menunjukkan keterlibatan timbal balik yang luar biasa di semua koneksi.",
                ...
                // Notifications
                uploadBothError:
                    "Harap unggah kedua file followers dan following",
                jsonOnlyError: "Harap unggah hanya file JSON",
                pdfGenerating: "Membuat Laporan...",
    ...
    updateAllTexts() {
        ...
        this.updateText('button[type="submit"] span', t.uploadBothFiles);
        ...
        this.updateText(".btn-primary span", t.downloadReport);
        this.updateText(".btn-secondary span", t.newAnalysis);
        ...
        if (submitBtn && !submitBtn.disabled) {
            this.updateText('button[type="submit"] span', t.analyzeData);
        }
    }
    ...
    updateText(selector, text) {
        ...
        if (element) {
            // Check if element has SVG icon before text
    ...
    updateLanguageToggle() {
        ...
        if (toggle) {
            ...
            const nextT = this.translations[nextLang];

            toggle.innerHTML = `
        <span class="language-flag">${
            this.currentLang === "en" ? "🇺🇸" : "🇮🇩"
        }</span>
        <span class="language-text">${t.currentLanguage}</span>
      `;

            toggle.title = `Switch to ${nextT.currentLanguage}`;
        }
    }
...
// Update PremiumFileUploadManager untuk support multi-bahasa
class PremiumFileUploadManager {
    // ... constructor dan methods lainnya tetap sama ...

    showError(message) {
        const langManager = window.languageManager;
        const translatedMessage = langManager
            ? langManager.t(message)
            : message;
        this.showNotification(translatedMessage, "error");
    }
    ...
    determineFileType(filename) {
        ...
        // Fallback dengan bahasa yang sesuai
        const promptText =
            langManager && langManager.currentLang === "id"
                ? `Apakah "${filename}" file followers atau following Anda? Ketik "followers" atau "following":`
                : `Is "${filename}" your followers or following file? Type "followers" or "following":`;

        const type = prompt(promptText);
        return type === "followers" ? "followers" : "following";
    }
