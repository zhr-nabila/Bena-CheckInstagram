...
class LanguageManager {
  constructor() {
    ...
    this.translations = {
      ...
      id: {
        ...
        heroSubtitle: "Ungkap siapa yang tidak follow balik dengan platform analisis canggih kami. Pribadi, aman, dan dirancang untuk mereka yang menghargai pengalaman premium.",
  ...
  updateAllTexts() {
    ...
    console.log('Updating texts to:', this.currentLang, t); // Debug log
  ...
  updateText(selector, text) {
    ...
    elements.forEach(element => {
      ...
      if (hasChildren) {
        // Preserve HTML structure, only update text content
  ...
  updateLanguageToggle() {
    ...
    if (toggle) {
      ...
      const nextText = nextLang === 'en' ? 'EN' : 'ID';
      ...
      toggle.title = `Switch to ${nextText}`;
    }
  }
...
class PremiumFileUploadManager {
  ...
  determineFileType(filename) {
    ...
    const promptText = langManager && langManager.currentLang === 'id' 
  ...
  checkFilesReady() {
    ...
    if (hasFollowers && hasFollowing) {
      submitBtn.disabled = false;
      submitBtn.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M21 11.01L3 11v2h18v-2zM3 16h12v2H3v-2zM21 6H3v2.01L21 8V6z"/></svg><span>' + t.analyzeData + '</span>';
    } else {
      submitBtn.disabled = true;
      submitBtn.innerHTML = '<span>' + t.uploadBothFiles + '</span>';
    }
  }
