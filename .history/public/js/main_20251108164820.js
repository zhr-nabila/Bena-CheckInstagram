...
class PremiumFileUploadManager {
    ...
    addFile(file) {
        ...
        // Jika sudah ada file dengan tipe yang sama, replace
        ...
        this.showSuccess(`✅ ${fileType.charAt(0).toUpperCase() + fileType.slice(1)} file uploaded successfully!`);
    }
    ...
    determineFileType(filename) {
        ...
        if (lowerName.includes('followers_1') || 
            lowerName.includes('follower') || 
            lowerName === 'followers.json') {
            return "followers";
        }
        if (lowerName.includes('following') || 
            lowerName === 'following.json') {
            return "following";
        }

        // Jika tidak jelas, tanya user dengan UI yang lebih baik
        const type = this.askFileType(filename);
        return type === "followers" ? "followers" : "following";
    }

    askFileType(filename) {
        ...
        modal.innerHTML = `
            <h3 style="margin-bottom: 1rem; color: #1a202c;">Select File Type</h3>
            <p style="margin-bottom: 1.5rem; color: #4a5568;">What type is "<strong>${filename}</strong>"?</p>
            <div style="display: flex; gap: 1rem; justify-content: center;">
                <button id="selectFollowers" style="padding: 0.75rem 1.5rem; border: none; border-radius: 8px; background: #4299e1; color: white; cursor: pointer;">Followers</button>
                <button id="selectFollowing" style="padding: 0.75rem 1.5rem; border: none; border-radius: 8px; background: #48bb78; color: white; cursor: pointer;">Following</button>
            </div>
        `;
        ...
        return new Promise((resolve) => {
            ...
            modal.querySelector('#selectFollowers').addEventListener('click', () => cleanup('followers'));
            modal.querySelector('#selectFollowing').addEventListener('click', () => cleanup('following'));
        });
    }
    ...
    createFileItem(fileType, file) {
        ...
        div.innerHTML = `
            ...
            <button class="file-remove" data-file-type="${fileType}" title="Remove file">×</button>
        `;
    ...
    removeFile(fileType) {
        ...
        this.updateFormInputs();
        this.checkFilesReady();
        this.showError(`❌ ${fileType.charAt(0).toUpperCase() + fileType.slice(1)} file removed`);
    }
    ...
    handleFormSubmit(e) {
        ...
        if (submitBtn) {
            submitBtn.disabled = true;
            submitBtn.innerHTML = '<div class="loading-spinner-small"></div><span>Analyzing...</span>';
        }
    }
        ...
        pdfForms.forEach((form) => {
            form.addEventListener("submit", async (e) => {
                ...
                try {
                    ...
                    const followersCount = form.querySelector(
                        'input[name="followersCount"]'
                    )?.value;
                    const followingCount = form.querySelector(
                        'input[name="followingCount"]'
                    )?.value;
                    const users = form.querySelector(
                        'input[name="users"]'
                    )?.value;

                    if (!followersCount || !followingCount || !users) {
                        throw new Error(
                    ...
                    if (response.ok) {
                        ...
                        const isIndonesian =
                            document
                                ...
                                ?.getAttribute("data-lang") === "id";
                        a.download = isIndonesian
                            ? "laporan_instagram_unfollowers.pdf"
                            : "instagram_unfollowers_report.pdf";
...
class SimpleAnimationManager {
    ...
    addLoadingSpinnerStyles() {
        // Add styles for loading spinners if not already present
    ...
    observeElements() {
        ...
        document
            .querySelectorAll(".upload-card, .results-card, .premium-features")
            .forEach((el) => {
                observer.observe(el);
            });
    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll('.btn, .upload-btn, .theme-toggle, .lang-btn').forEach(btn => {
