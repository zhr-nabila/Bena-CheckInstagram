...
class PremiumFileUploadManager {
    ...
    setupPDFDownload() {
    // Cari semua form PDF dan hapus event listener JavaScript
    const pdfForms = document.querySelectorAll('form[action*="download-pdf"]');
    pdfForms.forEach((form) => {
        // Hapus semua event listener dengan membuat form baru
        const newForm = form.cloneNode(true);
        form.parentNode.replaceChild(newForm, form);
        
        // Optional: Tambahkan small delay untuk memastikan
        setTimeout(() => {
            console.log('PDF form reset to natural behavior');
        }, 100);
    });
}
