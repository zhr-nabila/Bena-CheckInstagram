...
class PremiumFileUploadManager {
    ...
    determineFileType(filename) {
        ...
        if (
            lowerName.includes("followers_1") ||
            lowerName.includes("follower") ||
            lowerName === "followers.json"
        ) {
            return "followers";
        }
        if (lowerName.includes("following") || lowerName === "following.json") {
            return "following";
        }
    ...
    //                 // PERBAIKAN: Validasi yang lebih safe
...
class HeartCursorManager {
    constructor() {
        this.cursor = null;
        this.heart = null;
        this.trails = [];
        this.trailCount = 4; // Kurangi jumlah trail
        this.lastX = 0;
        this.lastY = 0;
        this.init();
    }
    ...
    createCursor() {
        ...
        this.cursor.appendChild(this.heart);
    ...
    bindEvents() {
        document.addEventListener('mousemove', (e) => {
            ...
            // Kurangi frekuensi trail untuk performa
            if (Math.abs(e.clientX - this.lastX) > 3 || Math.abs(e.clientY - this.lastY) > 3) {
    ...
    bindSpecialHoverEffects() {
        // Buttons
        const buttons = document.querySelectorAll('button, .btn, .upload-btn, .unfollow-btn');
        buttons.forEach(button => {
            button.addEventListener('mouseenter', () => {
                this.cursor.classList.add('hover', 'button-hover');
            });
            button.addEventListener('mouseleave', () => {
                this.cursor.classList.remove('hover', 'button-hover');
            });
        });
        ...
        links.forEach(link => {
            link.addEventListener('mouseenter', () => {
                this.cursor.classList.add('hover', 'link-hover');
            });
            link.addEventListener('mouseleave', () => {
                this.cursor.classList.remove('hover', 'link-hover');
            });
        });
        ...
        if (logo) {
            logo.addEventListener('mouseenter', () => {
                this.cursor.classList.add('hover', 'logo-hover');
            });
            logo.addEventListener('mouseleave', () => {
                this.cursor.classList.remove('hover', 'logo-hover');
            });
        }

        // Interactive elements
        const interactive = document.querySelectorAll('.theme-toggle, .lang-btn, .feature-card, .stat-card, .upload-area');
    ...
    animate() {
        this.trails.forEach(trail => {
            if (trail.active) {
                trail.age += 0.03; // Lebih cepat fade
                if (trail.age >= 1) {
                    trail.active = false;
                    trail.element.style.opacity = '0';
                } else {
                    const scale = 1 - trail.age;
                    trail.element.style.opacity = (1 - trail.age).toString();
